...
const ProfileScreen = ({ navigation }) => {
  ...
  const fetchUserData = async () => {
    try {
      setLoading(true);
      console.log("1. Iniciando busca de dados do usuário");
      
      // Primeiro tentar pegar o userId diretamente
      let userId = await AsyncStorage.getItem("userId");
      console.log("2. userId do AsyncStorage:", userId);
      
      // Se não encontrar, tentar pegar do objeto user
      if (!userId) {
        console.log("3. userId não encontrado, tentando buscar do objeto user");
        const userJson = await AsyncStorage.getItem("@BeSustainable:user");
        console.log("4. userJson encontrado:", userJson);
        
        if (userJson) {
          const user = JSON.parse(userJson);
          userId = user.id.toString();
          console.log("5. userId extraído do userJson:", userId);
          // Salvar o userId separadamente para futuras consultas
          await AsyncStorage.setItem("userId", userId);
          console.log("6. userId salvo no AsyncStorage");
        }
      }

      if (!userId) {
        console.log("7. Nenhum userId encontrado");
        throw new Error("Usuário não encontrado. Por favor, faça login novamente.");
      }

      console.log("8. Fazendo requisição para:", `${API_URL}/users/${userId}/level`);
      const response = await axios.get(`${API_URL}/users/${userId}/level`);
      console.log("9. Resposta da API:", response.data);
      
      setUserData(response.data);
      setError(null);
    } catch (err) {
      console.log("10. Erro capturado:", err);
      console.log("11. Tipo do erro:", err.constructor.name);
      console.log("12. Mensagem do erro:", err.message);
      if (err.response) {
        console.log("13. Dados da resposta de erro:", {
